 ...
 */
 ...
 /**
   ...
   * Generally the Clarke transform uses three-phase currents <code>I_A_ID, I_B_ID and I_C_ID</code> to calculate currents
   * in the two-phase orthogonal stator axis <code>Ialpha</code> and <code>Ibeta</code>.
   * When <code>Ialpha</code> is superposed with <code>I_A_ID</code> as shown in the figure below
   * \image html clarke.gif Stator current space vector and its components in (a,b).
   * and <code>I_A_ID + I_B_ID + I_C_ID = 0</code>, in this condition <code>Ialpha</code> and <code>Ibeta</code>
   * can be calculated using only <code>I_A_ID</code> and <code>I_B_ID</code>.
   *
   * The function operates on a single sample of data and each call to the function returns the processed output.
   * The library provides separate functions for Q31 and floating-point data types.
   * \par Algorithm
   * \image html clarkeFormula.gif
   * where <code>I_A_ID</code> and <code>I_B_ID</code> are the instantaneous stator phases and
   ...
   * @param[in]       I_A_ID       input three-phase coordinate <code>a</code>
   * @param[in]       I_B_ID       input three-phase coordinate <code>b</code>
  ...
  __STATIC_INLINE void arm_clarke_f32(
				      ...
				      float32_t * pIbeta)
  {
    /* Calculate pIalpha using the equation, pIalpha = I_A_ID */
    *pIalpha = I_A_ID;

    /* Calculate pIbeta using the equation, pIbeta = (1/sqrt(3)) * I_A_ID + (2/sqrt(3)) * I_B_ID */
    *pIbeta = ((float32_t) 0.57735026919 * I_A_ID + (float32_t) 1.15470053838 * I_B_ID);
				      ...
				      q31_t * pIbeta)
  {
    ...
    /* Calculating pIalpha from I_A_ID by equation pIalpha = I_A_ID */
    *pIalpha = I_A_ID;

    /* Intermediate product is calculated by (1/(sqrt(3)) * I_A_ID) */
    product1 = (q31_t) (((q63_t) I_A_ID * 0x24F34E8B) >> 30);

    /* Intermediate product is calculated by (2/sqrt(3) * I_B_ID) */
    product2 = (q31_t) (((q63_t) I_B_ID * 0x49E69D16) >> 30);
					  ...
					  q31_t * pIb)
  {
    ...
    *pIa = Ialpha;
    ...
    /* Intermediate product is calculated by (1/(2*sqrt(3)) * I_A_ID) */
